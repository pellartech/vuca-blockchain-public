...
import {
  ...
  L2ERC721Predicate,
  L2ERC721Predicate__factory,
  L1ERC721Predicate,
  L1ERC721Predicate__factory,
...
describe('Bridge Predicate ERC721 test', () => {
  ...
  beforeEach(async () => {
    ...
    const mockL1ERC721Factory = (await ethers.getContractFactory('TokenERC721', owner)) as TokenERC721__factory
    l1ERC721Mock = await mockL1ERC721Factory.deploy()
    await l1ERC721Mock.deployed()

    const factory2 = (await ethers.getContractFactory('L2ERC721Template', owner)) as L2ERC721Template__factory
    ...
    const initL2Predicate = async () => {
      ...
      const predicateImplFactory = (await ethers.getContractFactory('L2ERC721Predicate', owner)) as L2ERC721Predicate__factory
    ...
    const initL1Predicate = async () => {
      ...
      const l1PredicateFactory = (await ethers.getContractFactory('L1ERC721Predicate', owner)) as L1ERC721Predicate__factory
    ...
    await l1ERC721Mock.mint(owner.address, 10)
    await l1ERC721Mock.mint(bob.address, 10)

    await l1ERC721Mock.setApprovalForAll(l1TokenPredicate.address, true)
    await l1ERC721Mock.setApprovalForAll(l2TokenPredicate.address, true)
    await l1ERC721Mock.connect(bob).setApprovalForAll(l1TokenPredicate.address, true)
    await l1ERC721Mock.connect(bob).setApprovalForAll(l2TokenPredicate.address, true)
  })
  ...
  describe('Flow', () => {
    context('Test case for mapToken', () => {
      it('mapToken should be correct', async () => {
        await l1multisigContract.submitTransaction(l1TokenPredicate.address, 0, l1TokenPredicate.interface.encodeFunctionData('mapToken', [l1ERC721Mock.address]))
        const txnId = Number(await l1multisigContract.getTransactionCount()) - 1
        await l1multisigContract.executeTransaction(txnId)
        const expected = ethers.utils.getCreate2Address(await l1TokenPredicate.l2Predicate(), ethers.utils.solidityKeccak256(['address'], [l1ERC721Mock.address]), proxyByteCodeHash)
        expect(await l1TokenPredicate.l1ToL2Gateway(l1ERC721Mock.address)).to.be.eq(expected)

        const message = ethers.utils.defaultAbiCoder.encode(['address', 'uint256', 'address', 'string', 'string', 'uint8'], [l1multisigContract.address, 0, l1ERC721Mock.address, '', '', 18])
        ...
        expect(await l2TokenPredicate.l1ToL2Gateway(l1ERC721Mock.address)).to.be.eq(expected)

        let l2Token: any = L2ERC721Template__factory.connect(expected, bob)
        expect(await l2Token.rootToken()).equal(l1ERC721Mock.address)
        expect(await l2Token.predicate()).equal(l2TokenPredicate.address)

        l2Token = ContractProxy__factory.connect(expected, alice)
        await expect(l2Token.initialize(l1ERC721Mock.address, '0x')).reverted
      })
    })
    ...
    context('Test case for deposit', () => {
      it('deposit should be revert with paused', async () => {
        ...
        let message = ethers.utils.defaultAbiCoder.encode(['address', 'uint256', 'address', 'string', 'string', 'uint8'], [l1multisigContract.address, 0, l1ERC721Mock.address, '', '', 18])
        ...
        const l2Token = L2ERC721Template__factory.connect(expected, bob)
        expect(await l2Token.rootToken()).equal(l1ERC721Mock.address)
        expect(await l2Token.predicate()).equal(l2TokenPredicate.address)

        await expect(l1TokenPredicate.depositTo(l1ERC721Mock.address, bob.address, tokenIds)).revertedWith('Paused')

        await expect(l1TokenPredicate.deposit(l1ERC721Mock.address, tokenIds)).revertedWith('Paused')
      ...
      it('deposit should be correct', async () => {
        ...
        message = ethers.utils.defaultAbiCoder.encode(
          ...
          [owner.address, 0, l1ERC721Mock.address, expected, bob.address, tokenIds]
        )

        const txn = await l1TokenPredicate.depositTo(l1ERC721Mock.address, bob.address, tokenIds)
        ...
        expect(await l1ERC721Mock.balanceOf(owner.address)).equal(0)
        expect(await l2Token.balanceOf(bob.address)).equal(10)
      })
    })
    ...
    context('Test case for withdraw', () => {
      it('withdraw should be correct', async () => {
        ...
        await l1TokenPredicate.depositTo(l1ERC721Mock.address, bob.address, tokenIds)
        ...
        message = ethers.utils.defaultAbiCoder.encode(
          ...
          [bob.address, 0, l1ERC721Mock.address, expected, bob.address, tokenIds]
        )

        await pauseContract(l2multisigContract, l2TokenPredicate)
        await expect(l2TokenPredicate.connect(bob).withdrawTo(expected, bob.address, tokenIds)).revertedWith('Paused')
        await expect(l2TokenPredicate.connect(bob).withdraw(expected, tokenIds)).revertedWith('Paused')
        await unpauseContract(l2multisigContract, l2TokenPredicate)

        const txn = await l2TokenPredicate.connect(bob).withdrawTo(expected, bob.address, tokenIds)
        ...
        expect(await l1ERC721Mock.balanceOf(bob.address)).equal(20)
      })
    })
    ...
    context('Test case for delegacy', () => {
      ...
      it('delegacyApproveForAll should be correct', async () => {
        ...
        await l2Token.connect(alice).delegacyApproveForAll(value, signature, systemSignature)
