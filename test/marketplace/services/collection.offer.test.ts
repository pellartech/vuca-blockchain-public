...
describe('Marketplace CollectionOffer test', () => {
  const serviceId = ethers.utils.solidityKeccak256(['string'], ['CollectionOffer'])
  ...
  beforeEach(async () => {
    ...
    // deploy service
    const serviceFactory = (await ethers.getContractFactory('CollectionOffer', owner)) as CollectionOffer__factory
  ...
  context('Test case for initialize', () => {
    it('initialize should be correct', async () => {
      ...
      expect(await service.COLLECTION_OFFERING_ID_COUNTER_KEY()).to.be.eq(ethers.utils.hexlify(ethers.utils.toUtf8Bytes('COLLECTION_OFFERING_ID_COUNTER')))
      expect(await service.COLLECTION_OFFERING_ID_FULFILLED_COUNTER_KEY()).to.be.eq(ethers.utils.hexlify(ethers.utils.toUtf8Bytes('COLLECTION_OFFERING_ID_FULFILLED_COUNTER_KEY')))
      expect(await service.LISTING_COLLECTION_OFFER_ITEM_PREFIX()).to.be.eq(ethers.utils.hexlify(ethers.utils.toUtf8Bytes('LISTING_COLLECTION_OFFER_ITEM_')))
      expect(await service.LISTED_COLLECTION_OFFER_ITEM_KEY()).to.be.eq(ethers.utils.hexlify(ethers.utils.toUtf8Bytes('LISTED_COLLECTION_OFFER_ITEM')))
      expect(await service.CANCELLED_COLLECTION_OFFER_ITEM_KEY()).to.be.eq(ethers.utils.hexlify(ethers.utils.toUtf8Bytes('CANCELLED_COLLECTION_OFFER_ITEM')))
      expect(await service.FULFILLED_COLLECTION_OFFER_ITEM_KEY()).to.be.eq(ethers.utils.hexlify(ethers.utils.toUtf8Bytes('FULFILLED_COLLECTION_OFFER_ITEM')))
  ...
  context('Test case for offerItem', () => {
    ...
    it('offerItem should be correct', async () => {
      ...
      expect(decodedEvent.action).equal(ethers.utils.solidityKeccak256(['bytes'], [await service.LISTED_COLLECTION_OFFER_ITEM_KEY()]))
      ...
      expect(_listing.item.items[0].id.toNumber()).equal(0)
      ...
      let res = await router.forwardStaticcallRequest(
        ...
        keeper.interface.encodeFunctionData('getUint256', [
          serviceId, //
          await service.COLLECTION_OFFERING_ID_COUNTER_KEY(),
        ])
      )
      ...
      expect(decodedEvent.serviceId).equal(serviceId)
      expect(decodedEvent.action).equal(ethers.utils.solidityKeccak256(['bytes'], [await service.UPDATED_COLLECTION_OFFER_ITEM_KEY()]))
      // expect(decodedEvent.dataStructure).equal(listedTuple)
  ...
  context('Test case for cancelOffer', () => {
    ...
    it('cancelOffer should be correct', async () => {
      ...
      expect(decodedEvent.serviceId).equal(serviceId)
      expect(decodedEvent.action).equal(ethers.utils.solidityKeccak256(['bytes'], [await service.CANCELLED_COLLECTION_OFFER_ITEM_KEY()]))
  ...
  context('Test case for fullfillItem', () => {
    ...
    it('fullfillItem should be correct', async () => {
      ...
      const decodedEvent = keeper.interface.decodeEventLog('EventFired', event.data, event.topics)
      expect(decodedEvent.serviceId).equal(serviceId)
      expect(decodedEvent.action).equal(ethers.utils.solidityKeccak256(['bytes'], [await service.FULFILLED_COLLECTION_OFFER_ITEM_KEY()]))
    ...
    it('fullfillItem should be correct', async () => {
      ...
      await mock20.connect(john).approve(keeper.address, 10000)
      const endTime = moment().add(1, 'days').unix()
      ...
      let tx = await (await router.forwardRequest(serviceId, service.interface.encodeFunctionData('fullfillItem', [value, [signature, systemSignature]]))).wait()
      let event = find(tx.logs, (log) => log.topics[0] === keeper.interface.getEventTopic('EventFired'))!
      let decodedEvent = keeper.interface.decodeEventLog('EventFired', event.data, event.topics)
      expect(decodedEvent.serviceId).equal(serviceId)
      expect(decodedEvent.action).equal(ethers.utils.solidityKeccak256(['bytes'], [await service.FULFILLED_COLLECTION_OFFER_ITEM_KEY()]))
      ...
      expect(_listing.runtime.startTime.toNumber()).equal(0)
      expect(_listing.runtime.endTime.toNumber()).equal(endTime)
      expect(_listing.fulfilledId.toNumber()).equal(0)
      let fulfilledAmounts = _listing.fulfilled
      expect(fulfilledAmounts[0].toNumber()).equal(5)
      ...
      let res = await router.forwardStaticcallRequest(
        ...
        keeper.interface.encodeFunctionData('getUint256', [
          ...
          ethers.utils.defaultAbiCoder.encode(
            ...
            [await service.COLLECTION_OFFERING_ID_FULFILLED_BY_TOKEN_KEY(), 0, mock1155.address]
          ),
        ])
      )
      expect(res[0]).equal(true)
      expect(ethersAbi.decode(['uint256'], res[1])[0]).equal(5)
      ...
      tx = await (await router.forwardRequest(serviceId, service.interface.encodeFunctionData('fullfillItem', [value, [signature, systemSignature]]))).wait()
      event = find(tx.logs, (log) => log.topics[0] === keeper.interface.getEventTopic('EventFired'))!
      decodedEvent = keeper.interface.decodeEventLog('EventFired', event.data, event.topics)
      expect(decodedEvent.serviceId).equal(serviceId)
      expect(decodedEvent.action).equal(ethers.utils.solidityKeccak256(['bytes'], [await service.FULFILLED_COLLECTION_OFFER_ITEM_KEY()]))
      ...
      expect(_listing.runtime.startTime.toNumber()).equal(0)
      expect(_listing.runtime.endTime.toNumber()).equal(endTime)
      expect(_listing.fulfilledId.toNumber()).equal(1)
      fulfilledAmounts = _listing.fulfilled
      expect(fulfilledAmounts[0].toNumber()).equal(10)
      ...
      expect(res[0]).equal(true)
      expect(ethersAbi.decode(['uint256'], res[1])[0]).equal(10)
      ...
      res = await router.forwardStaticcallRequest(
        ...
        keeper.interface.encodeFunctionData('getUint256', [
          ...
          ethers.utils.defaultAbiCoder.encode(
            ['bytes', 'uint256'], //
            [await service.COLLECTION_OFFERING_ID_FULFILLED_COUNTER_KEY(), 0]
          ),
        ])
      )
      expect(res[0]).equal(true)
      ...
      expect(await mock20.balanceOf(alice.address)).equal(1000)
      expect(await mock20.balanceOf(john.address)).equal(100)
    })
  })
})
