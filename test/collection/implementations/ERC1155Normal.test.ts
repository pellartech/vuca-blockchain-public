...
import { ContractProxy__factory, ERC1155Normal, ERC1155Normal__factory } from '../../../typechain-types'
...
const iFace = new ethers.utils.Interface(ERC1155Normal__factory.abi)
...
describe('AdotFactory test ERC1155', () => {
  ...
  let mainContract: ERC1155Normal
  ...
  beforeEach(async () => {
    ...
    const erc1155ImplFactory = (await ethers.getContractFactory('ERC1155Normal', owner)) as any
    const erc1155ImplContract = await erc1155ImplFactory.deploy()
    await erc1155ImplContract.deployed()

    const hash = ethers.utils.solidityKeccak256(['string'], ['ERC1155Normal'])
    await factoryContractIns.addImplementation(hash, erc1155ImplContract.address)
    ...
    mainContract = erc1155ImplContract.attach(expected)
  })

  context('Test case for initialize', () => {
    it('init data should be correct', async () => {
      expect(await mainContract.name()).eq('ERC1155')
      expect(await mainContract.symbol()).eq('ERC1155')
  ...
  context('Test case for setPhaseCondition', () => {
    it('setPhaseCondition should be revert without owner', async () => {
      await expect(mainContract.connect(bob).setPhaseCondition(0, '0x', '0x')).revertedWith('Ownable: caller is not the owner')
      await expect(mainContract.connect(bob).updatePhaseCondition(0, 0, '0x', '0x')).revertedWith('Ownable: caller is not the owner')
      await expect(mainContract.connect(bob).resetEligible(0, 0)).revertedWith('Ownable: caller is not the owner')
    })
    ...
    it('setPhaseCondition should be correct', async () => {
      ...
      await mainContract.setPhaseCondition(tokenId, config, signature)

      let phaseCondition = await mainContract.getPhaseCondition(tokenId, 0)
      ...
      await mainContract.updatePhaseCondition(tokenId, 0, config, signature)
      phaseCondition = await mainContract.getPhaseCondition(tokenId, 0)
      expect(phaseCondition[0]).eq(0)
      ...
      await mainContract.resetEligible(1, 0)
      phaseCondition = await mainContract.getPhaseCondition(tokenId, 0)
      expect(phaseCondition[0]).eq(1)

      await mainContract.deletePhaseCondition(1, 0)
      phaseCondition = await mainContract.getPhaseCondition(tokenId, 0)
  ...
  context('Test case for claim', () => {
    it('claim should be revert without phase', async () => {
      await expect(mainContract.claim(1, 0, 10, '0x')).revertedWith('Invalid phase')
    })
    ...
    it('claim with public phase should be correct', async () => {
      ...
      await mainContract.connect(bob).claim(tokenId, 0, 10, '0x', {
      ...
      expect(await mainContract.totalSupply(tokenId)).eq(10)
      expect(await mainContract.balanceOf(bob.address, tokenId)).eq(10)

      expect(await mainContract.getPhaseConditionClaimed(tokenId, 0, bob.address)).eq(10)
      expect(await mainContract.getPhaseConditionClaimedWithVersion(tokenId, 0, 0, bob.address)).eq(10)
      expect(await mainContract.getPhaseTotalClaimed(tokenId, 0)).eq(10)
      expect(await mainContract.getPhaseTotalClaimedWithVersion(tokenId, 0, 0)).eq(10)

      await expect(
        mainContract.connect(bob).claim(tokenId, 0, 1, '0x', {
          value: ethers.utils.parseEther('0.1').mul(1),
        })
      ).revertedWith('Exceeds max per wallet')

      await mainContract.resetEligible(tokenId, 0)

      await mainContract.connect(bob).claim(tokenId, 0, 10, '0x', {
        value: ethers.utils.parseEther('0.1').mul(10),
      })

      expect(await mainContract.totalSupply(tokenId)).eq(20)
      expect(await mainContract.getPhaseConditionClaimed(tokenId, 0, bob.address)).eq(10)
      expect(await mainContract.getPhaseConditionClaimedWithVersion(tokenId, 0, 1, bob.address)).eq(10)
      expect(await mainContract.getPhaseTotalClaimed(tokenId, 0)).eq(10)
      expect(await mainContract.getPhaseTotalClaimedWithVersion(tokenId, 0, 1)).eq(10)

      await expect(
        mainContract.connect(alice).claim(tokenId, 0, 10, '0x', {
          value: ethers.utils.parseEther('0.1').mul(10),
        })
      ).revertedWith('Exceeds max')

      await mainContract.resetEligible(tokenId, 0)
      await mainContract.deletePhaseCondition(tokenId, 0)

      await expect(
        mainContract.connect(bob).claim(tokenId, 0, 10, '0x', {
          value: ethers.utils.parseEther('0.1').mul(10),
        })
      ).revertedWith('Phase removed')

      const tokenURI = await mainContract.uri(0)
    ...
    it('claim with private phase should be correct', async () => {
      ...
      await mainContract.connect(bob).claim(tokenId, 0, 10, signature, {
        value: ethers.utils.parseEther('0.1').mul(10),
      })
    ...
    it('claimTo with public phase should be correct', async () => {
      ...
      await mainContract.connect(bob).claimTo(john.address, tokenId, 0, 10, '0x', {
        value: ethers.utils.parseEther('0.1').mul(10),
      })
      ...
      expect(await mainContract.totalSupply(tokenId)).eq(10)
      expect(await mainContract.balanceOf(john.address, tokenId)).eq(10)
    ...
    it('claimTo with private phase should be correct', async () => {
      ...
      await mainContract.connect(bob).claimTo(john.address, tokenId, 0, 10, signature, {
        value: ethers.utils.parseEther('0.1').mul(10),
      })
  ...
  context('Test case for delegacyClaim', () => {
    it('delegacyClaim with public phase should be correct', async () => {
      ...
      const calldata = ethers.utils.defaultAbiCoder.encode(['uint256', 'uint256', 'uint256', 'bytes'], [tokenId, 0, 10, wlSignature])
